...
const ChangePassword = () => {
  ...
  const handleChangePassword = async ()=>{
    ...
    try {
      setIsLoading(true);
      const res= await axios.post(`${import.meta.env.VITE_REACT_BACKEND_BASEURL}/user/change-password/${email}`, {
        newPassword,
        confirmPassword
      })
  ...
  return (
    <div className='min-h-screen flex items-center justify-center bg-blue-100 px-4'>
      <div className='bg-white shadow-md rounded-lg p-6 max-w-md w-full'>
        ...
        {success && <p className='text-blue-500 text-sm mb-4 text-center'>{success}</p>}
