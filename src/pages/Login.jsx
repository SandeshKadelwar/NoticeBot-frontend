...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      const res = await axios.post(`${process.env.BACKEND_BASEURL}/user/login`, formData, {
        headers: {
        ...
        <div className='flex flex-1 items-center justify-center px-4'>
          <div className='max-w-md w-full space-y-6'>
            <div className='space-y-2'>
              <h1 className='text-3xl font-bold tracking-tight text-blue-700'>Login into your account</h1>
              <p className='text-gray-600'>Start organizing your thoughts and ideas today.</p>
            </div>
            <Card className="w-full max-w-sm">
              <CardHeader className='space-y-1'>
                <CardTitle className='text-2xl text-center text-blue-700'>Login</CardTitle>
              ...
              <CardFooter className="flex-col gap-2">
                ...
                <div>
                  <p>
                    ...
                    <Link to={'/signup'} className='text-blue-600 hover:underline font-medium relative'>Create one</Link>
                  </p>
                </div>
              </CardFooter>
            </Card>
